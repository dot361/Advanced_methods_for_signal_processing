...
def generateData():
    ...
    np.save('./signal_waves_medium_2048_200k.npy', samples)
    return Fs

def applyNoise():
    # Load data
    data = np.load('./signal_waves_medium_2048_200k.npy')
    x_val, y_val = data[:,0], data[:,1]

    # Add noise to data
    noisy_samples = []
    for i in range(0, len(x_val)):
        SNR = -np.random.randint(10,25)
        # print(SNR)
        if i % 1000 == 0:
            print(i)
            print(SNR)
    ...
    # Save data to file for re-use
    np.save('./signal_waves_noisy_2048_200k.npy', noisy_samples)

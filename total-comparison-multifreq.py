...
def mainFun(RMSE_array, sigma_array, model):
    x, y =generateSignal(N,T,F, extraF=extraF)
    ...
    xpeak, _ = find_peaks(fft_originalY, height=0.3)
    # plt.plot(fft_originalX, fft_originalY)
    # fig = go.Figure()
    iter_rmse = []
    iter_sigma = []
    print(SNR)
    ...
    sigma_FFT = [np.max(fft_y[xpeak[0]-3:xpeak[0]+3])/np.mean(fft_y), np.max(fft_y[xpeak[1]-3:xpeak[1]+3])/np.mean(fft_y)]

    rmse_rez_FFT = rmse(fft_y, fft_originalY)
    # plt.subplot(221)
    # plt.plot(fft_originalX, fft_originalY, label='Pure')

    # plt.plot(fft_x,fft_y, label='FFT '+r"$\sigma$ = "+str(np.round(sigma_FFT, 3)))
    ...
    sigma_ML = [np.max(fft_ML_y[xpeak[0]-3:xpeak[0]+3])/np.mean(fft_ML_y), np.max(fft_ML_y[xpeak[1]-3:xpeak[1]+3])/np.mean(fft_ML_y)]

    rmse_rez_ML = rmse(fft_ML_y, fft_originalY)
    # plt.subplot(221)
    # plt.plot(fft_originalX, fft_originalY, label='Pure')
    # plt.plot(fft_x,fft_y, label='FFT '+r"$\sigma$ = "+str(np.round(sigma_FFT, 3)))
    # plt.plot(fft_ML_x, fft_ML_y, label='ML'+r"$\sigma$ = "+str(np.round(sigma_ML,3)))
    # plt.xlabel('Frequency [Hz]')
    # plt.ylabel("Amplitude")
    ...
    sigma_SSA = [np.max(yf[xpeak[0]-3:xpeak[0]+3])/np.mean(yf), np.max(yf[xpeak[1]-3:xpeak[1]+3])/np.mean(yf)]

    rmse_rez_SSA = rmse(yf, fft_originalY)
    # plt.subplot(222)
    # plt.plot(fft_originalX, fft_originalY, label='Pure')
    # plt.plot(fft_x,fft_y, label='FFT '+r"$\sigma$ = "+str(np.round(sigma_FFT, 3)))
    # plt.plot(xf,yf, label='SSA 256,6'+r"$\sigma$ = "+str(np.round(sigma_SSA,3)))
    # plt.xlabel('Frequency [Hz]')
    # plt.ylabel("Amplitude")
    # plt.legend(loc='upper right', prop={'size': 8})
    # plt.grid()
    ...
    # plt.plot(fft_originalX, fft_originalY, label='Pure')
    # plt.plot(fft_x,fft_y, label='FFT '+r"$\sigma$ = "+str(np.round(sigma_FFT, 3)))
    # plt.plot(xf,yf, label='SSA 8,6'+r"$\sigma$ = "+str(np.round(sigma_SSA,3)))
    # plt.xlabel('Frequency [Hz]')
    # plt.ylabel("Amplitude")
    # plt.legend(loc='upper right', prop={'size': 8})
    # plt.grid()
    ...
    # plt.plot(fft_originalX, fft_originalY, label='Pure')
    # plt.plot(fft_x,fft_y, label='FFT '+r"$\sigma$ = "+str(np.round(sigma_FFT, 3)))
    # plt.plot(xf,yf, label='SSA 8,2'+r"$\sigma$ = "+str(np.round(sigma_SSA,3)))
    # plt.xlabel('Frequency [Hz]')
    # plt.ylabel("Amplitude")
    # plt.legend(loc='upper right', prop={'size': 8})
    # plt.grid()
...
labels = ["FFT", "ML", "SSA 256,6", "SSA 8,6", "SSA 8,2"]
temp = np.load('rmse-multifreq.npy')
temp = np.mean(temp,axis=0)

plt.plot(np.arange(-40,-9),temp)
plt.xlabel("dB")
plt.ylabel("RMSE")
plt.grid()
plt.minorticks_on()
...
# plt.plot(np.arange(-40,-9),temp.T[0].T)
# plt.grid()
# plt.legend(labels, loc='upper left', prop={'size': 10})
# plt.subplot(122)

# plt.plot(np.arange(-40,-9),temp.T[1].T)
# plt.xlabel("dB")
...
for j in range(0,1001):
    ...
    np.save('rmse-multifreq', np.array(total_rmse))
    np.save('sigma-multifreq', np.array(total_sigma))
